...
class ConversationalNutritionistAgent(Agent):
    ...
    def __init__(self, model_id: str = "meta-llama/llama-4-scout-17b-16e-instruct", user_context: str = "", allergies: str = "", dietary_preferences: str = "", knowledge=None):
        ...
        instructions = [
            ...
            "Quando l'utente chiede cosa mangiare (cena, pranzo, colazione, spuntino), esegui internamente questi passi e poi rispondi UNA SOLA VOLTA. NON raccontare i passi, non nominare strumenti né knowledge base: all'utente arriva solo la risposta finale unica.",
            "STEP 1 - RICETTA PRINCIPALE DAL RAG: cerca PRIMA nella knowledge base e costruisci UNA ricetta principale fondata su quei dati nutrizionali (valori, porzioni, linee guida), calibrata sui macro RIMANENTI della fascia. Cita la fonte del dato preso dalla knowledge base.",
            "STEP 2 - ALTERNATIVE ONLINE: POI chiama SEMPRE lo strumento di ricerca ricette (obbligatorio, mai saltarlo). Costruisci la query per ingredienti e tipo di pasto coerenti col fabbisogno (es. 'cena salmone verdure proteica', senza numeri). Prendi ALMENO 3 alternative dai risultati, ognuna con TITOLO e LINK alla fonte.",
            "STEP 3 - RISPOSTA UNICA: unisci tutto in UN SOLO messaggio, in quest'ordine: (a) '**La mia proposta:**' con la ricetta principale del RAG (ingredienti, grammature, macro stimati); (b) '**Altre 3 alternative da provare online:**' con le 3+ ricette trovate, ognuna titolo + link + macro stimati e perché rientra nel fabbisogno. NON dare due risposte separate: una sola risposta completa.",
            "",
            ...
            "- 🔴 UNA SOLA RISPOSTA. Quando devi cercare una ricetta, chiama lo strumento di ricerca PRIMA di scrivere qualsiasi cosa. È VIETATO scrivere una ricetta 'provvisoria', 'di esempio' o una prima analisi prima della ricerca: genera due risposte doppie e contraddittorie (errore grave). Aspetta i risultati, poi scrivi la risposta finale, una sola volta.",
